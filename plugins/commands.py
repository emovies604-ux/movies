            ...
            if refData and refData.split("-", 1)[0] == "Jisshu":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        buttons = [[
                            ...
                            InlineKeyboardButton("Hᴇʟᴘ ⚙️", callback_data=''),
                            InlineKeyboardButton('Aʙᴏᴜᴛ 💌', callback_data=f'')
                        ],[
                            InlineKeyboardButton('Pʀᴇᴍɪᴜᴍ 🎫', callback_data=''),
                            InlineKeyboardButton('Rᴇғᴇʀ ⚜️', callback_data="")
                        ],[
                            InlineKeyboardButton('Mᴏsᴛ Sᴇᴀʀᴄʜ 🔍', callback_data=""),
                            InlineKeyboardButton('Tᴏᴘ Tʀᴇɴᴅɪɴɢ ⚡', callback_data="")
                        ]]
                        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            buttons = [[
                ...
                InlineKeyboardButton(text="😁 ʙᴜʏ sᴜʙsᴄʀɪᴘᴛɪᴏɴ - ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😁", callback_data=''),
            ]]
            reply_markup=InlineKeyboardMarkup(buttons)
            if await db.user_verified(user_id): 
...
async def refer(bot, message):
    btn = [[
        InlineKeyboardButton('invite link', url=f''),
